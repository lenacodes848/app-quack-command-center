...
const RULES = [
  ...
  {
    rule: 'email-address',
    pattern: /[A-Za-z0-9._%+-]+@(?!example\.(?:com|org|net)\b)[A-Za-z0-9-]+(?:\.[A-Za-z0-9-]+)*\.[A-Za-z]{2,}/,
  },
];
...
if (isMain) {
  ...
  const denylist = loadDenylist(process.env.SOURCE_PROTECTION_DENYLIST ?? join(rootDir, '.source-protection-denylist'));
  ...
  if (findings.length > 0) {
    ...
    console.error(`source protection scan FAILED: ${findings.length} finding(s). Matched text is not printed.`);
    process.exit(1);
  }
